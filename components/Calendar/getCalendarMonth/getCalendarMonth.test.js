...
const january1970Array = [[
  ...
  moment({ day: 8, year: 1970, month: 1 }),
]];

describe('getPreDayCount', () => {
  it('returns 3 days for January 1970', () => {
    expect(getPreDayCount(january1970Moment)).toEqual(3);
  });
});

describe('getPostDayCount', () => {
  it('returns 8 days for January 1970', () => {
    expect(getPostDayCount(january1970Moment)).toEqual(8);
  });
});

describe('getCalendarMonthArray', () => {
  it('produces a calendar month of dates ', () => {
    expect(getCalendarMonth(january1970Moment, 3, 8).toString())
      .toEqual(january1970Array.toString());
  });

  it('returns valid moment dates', () => {
    expect(getCalendarMonth(january1970Moment, 3, 8)[0][0].isValid()).toBe(true);
  });
});
